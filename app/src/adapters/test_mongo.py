...
class TestMongoReportRepository(unittest.TestCase):
    ...
    def test_save_record(self):
        ...
        self.assertEqual(
                _records_to_reports(
                    repository.db["problem_records"].find()
                    )[0],
                report1
                )
        repository.client.drop_database(DBNAME)
    ...
    def test_find_reports_by_kv(self):
        ...
        self.assertEqual(
                len(repository.find_reports_by_kv(data1)),
                2
                )
        self.assertEqual(
                repository.find_reports_by_kv(data2),
                [report1]
                )
        self.assertEqual(
                len(repository.find_reports_by_kv(data3)),
                3
                )
        # non existent
        data4 = {"anything": "else"}
        self.assertEqual(
                repository.find_reports_by_kv(data4),
                []
                )
        # weird data
        data5 = {"something_weird": {"yes": True, "No": False}, "key1":1}
        self.assertEqual(
                repository.find_reports_by_kv(data5),
                [report1]
                )
        repository.client.drop_database(DBNAME)
    ...
    def test_find_reports_by_hash(self):
        ...
        repository.save_report(report1)
        self.assertEqual(
                repository.find_reports_by_hash(report1.hash),
                [report1, report1]
                )
        # find non existent
        self.assertEqual(
                repository.find_reports_by_hash(123123123),
                []
                )
        repository.client.drop_database(DBNAME)
